...
-   입력 행을 반복하면서 추가 작업을 수행해야 하는 경우에만 **\<inputbuffer>_ProcessInput**을 재정의합니다. 예를 들어 `EndOfRowset` 모든 행이 처리 된 후 다른 작업을 수행 하기 위해를 테스트 해야 합니다. ** \<inputbuffer> _ProcessInputRow** 를 호출 하 여 행 처리를 수행 합니다.
