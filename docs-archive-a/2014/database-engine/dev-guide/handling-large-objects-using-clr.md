...
ms.locfileid: "87742512"
---
# <a name="handling-large-objects-using-clr"></a><span data-ttu-id="7986f-102">CLR을 사용하여 큰 개체 처리</span><span class="sxs-lookup"><span data-stu-id="7986f-102">Handling Large Objects Using CLR</span></span>
  <span data-ttu-id="7986f-103">SQL Server의 `HandlingLOBUsingCLR` 예제에서는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 및 CLR(공용 언어 런타임) 저장 프로시저를 사용하여 서버에서 사용할 수 있는 파일 시스템 간의 LOB(Large Object) 전송을 보여 줍니다.</span><span class="sxs-lookup"><span data-stu-id="7986f-103">The `HandlingLOBUsingCLR` sample for SQL Server demonstrates transferring large objects (LOBs) between [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] and a file system that is available to the server by using common language runtime (CLR) stored procedures.</span></span> <span data-ttu-id="7986f-104">이 예제에서는 서버 쪽 코드의 파일에 액세스하고 CLR 기반 저장 프로시저로부터 동적 쿼리와 저장 프로시저를 모두 호출하는 방법을 보여 줍니다.</span><span class="sxs-lookup"><span data-stu-id="7986f-104">This sample demonstrates how to access files in server-side code, and then invoke both dynamic queries and stored procedures from CLR-based stored procedures.</span></span> <span data-ttu-id="7986f-105">또한 [!INCLUDE[tsql](../../includes/tsql-md.md)]을 사용하여 CLR 메서드와 어셈블리를 등록 및 등록 해제하는 방법을 보여 줍니다.</span><span class="sxs-lookup"><span data-stu-id="7986f-105">It also demonstrates how to unregister and register CLR methods and assemblies by using [!INCLUDE[tsql](../../includes/tsql-md.md)].</span></span>  
  
## <a name="prerequisites"></a><span data-ttu-id="7986f-106">전제 조건</span><span class="sxs-lookup"><span data-stu-id="7986f-106">Prerequisites</span></span>  
 <span data-ttu-id="7986f-107">이 프로젝트를 만들고 실행하려면 다음 소프트웨어가 설치되어 있어야 합니다.</span><span class="sxs-lookup"><span data-stu-id="7986f-107">To create and run this project the following the following software must be installed:</span></span>  
  
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] <span data-ttu-id="7986f-108">또는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express</span><span class="sxs-lookup"><span data-stu-id="7986f-108">or [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span></span> <span data-ttu-id="7986f-109">[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express 설명서 및 예제 [웹 사이트](https://www.microsoft.com/download/details.aspx?id=42299)에서 무료로 구할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="7986f-109">You can obtain [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express free of charge from the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express Documentation and Samples [Web site](https://www.microsoft.com/download/details.aspx?id=42299)</span></span>  
  
-   <span data-ttu-id="7986f-110">[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 개발자 [웹 사이트](https://archive.codeplex.com/?p=SqlServerSamples)에서 제공되는 AdventureWorks 데이터베이스</span><span class="sxs-lookup"><span data-stu-id="7986f-110">The AdventureWorks database that is available at the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Developer [Web site](https://archive.codeplex.com/?p=SqlServerSamples)</span></span>  
  
-   <span data-ttu-id="7986f-111">.NET Framework SDK 2.0 이상 또는 Microsoft Visual Studio 2005 이상.</span><span class="sxs-lookup"><span data-stu-id="7986f-111">.NET Framework SDK 2.0 or later or Microsoft Visual Studio 2005 or later.</span></span> <span data-ttu-id="7986f-112">.NET Framework SDK는 무료로 구할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="7986f-112">You can obtain .NET Framework SDK free of charge.</span></span>  
  
-   <span data-ttu-id="7986f-113">또한 다음 조건을 충족해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="7986f-113">In addition, the following conditions must be met:</span></span>  
  
-   <span data-ttu-id="7986f-114">사용하고 있는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 인스턴스에 대해 CLR 통합이 설정되어 있어야 합니다.</span><span class="sxs-lookup"><span data-stu-id="7986f-114">The [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using must have CLR integration enabled.</span></span>  
  
-   <span data-ttu-id="7986f-115">CLR 통합을 설정하려면 다음 단계를 수행합니다.</span><span class="sxs-lookup"><span data-stu-id="7986f-115">In order to enable CLR integration, perform the following steps:</span></span>  
  
    #### <a name="enabling-clr-integration"></a><span data-ttu-id="7986f-116">CLR 통합 사용</span><span class="sxs-lookup"><span data-stu-id="7986f-116">Enabling CLR Integration</span></span>  
  
    -   <span data-ttu-id="7986f-117">다음 [!INCLUDE[tsql](../../includes/tsql-md.md)] 명령을 실행합니다.</span><span class="sxs-lookup"><span data-stu-id="7986f-117">Execute the following [!INCLUDE[tsql](../../includes/tsql-md.md)] commands:</span></span>  
    ...
    > [!NOTE]  
    >  <span data-ttu-id="7986f-118">CLR을 사용하도록 설정하려면 `ALTER SETTINGS` 서버 수준 사용 권한이 있어야 합니다. 이 사용 권한은 `sysadmin` 및 `serveradmin` 고정 서버 역할의 멤버가 암시적으로 소유합니다.</span><span class="sxs-lookup"><span data-stu-id="7986f-118">To enable CLR, you must have `ALTER SETTINGS` server level permission, which is implicitly held by members of the `sysadmin` and `serveradmin` fixed server roles.</span></span>  
  
-   <span data-ttu-id="7986f-119">사용하고 있는 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 인스턴스에 AdventureWorks 데이터베이스를 설치해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="7986f-119">The AdventureWorks database must be installed on the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using.</span></span>  
  
-   <span data-ttu-id="7986f-120">사용 중인 인스턴스의 관리자가 아닌 경우 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 설치를 완료 하려면 관리자에 게 **createassembly** 권한을 부여 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="7986f-120">If you are not an administrator for the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using, you must have an administrator grant you **CreateAssembly**  permission to complete the installation.</span></span>  
  
## <a name="building-the-sample"></a><span data-ttu-id="7986f-121">예제 빌드</span><span class="sxs-lookup"><span data-stu-id="7986f-121">Building the Sample</span></span>  
  
#### <a name="create-and-run-the-sample-by-using-the-following-instructions"></a><span data-ttu-id="7986f-122">다음 지침을 사용하여 예제를 만들고 실행합니다.</span><span class="sxs-lookup"><span data-stu-id="7986f-122">Create and run the sample by using the following instructions:</span></span>  
  
1.  <span data-ttu-id="7986f-123">Visual Studio 또는 .NET Framework 명령 프롬프트를 엽니다.</span><span class="sxs-lookup"><span data-stu-id="7986f-123">Open a Visual Studio or .NET Framework command prompt.</span></span>  
  
2.  <span data-ttu-id="7986f-124">필요한 경우 예제에 대한 디렉터리를 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="7986f-124">If necessary, create a directory for your sample.</span></span> <span data-ttu-id="7986f-125">이 예에서는 C:\MySample을 사용합니다.</span><span class="sxs-lookup"><span data-stu-id="7986f-125">For this example, we will use C:\MySample.</span></span>  
  
3.  <span data-ttu-id="7986f-126">이 예에는 서명된 어셈블리가 필요하므로 다음 명령을 입력하여 비대칭 키를 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="7986f-126">Since this example requires a signed assembly, create an asymmetric key by typing the command:</span></span>  
  
## <a name="sample-code"></a><span data-ttu-id="7986f-127">샘플 코드</span><span class="sxs-lookup"><span data-stu-id="7986f-127">Sample Code</span></span>  
 <span data-ttu-id="7986f-128">다음은 이 예제에 대한 코드 목록입니다.</span><span class="sxs-lookup"><span data-stu-id="7986f-128">The following are the code listings for this sample.</span></span>  
  
 <span data-ttu-id="7986f-129">C#</span><span class="sxs-lookup"><span data-stu-id="7986f-129">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="7986f-130">Visual Basic</span><span class="sxs-lookup"><span data-stu-id="7986f-130">Visual Basic</span></span>  
...
```  
 ...
 <span data-ttu-id="7986f-132">다음 [!INCLUDE[tsql](../../includes/tsql-md.md)] (`test.sql`)에서는 저장 프로시저를 실행하여 예를 테스트합니다.</span><span class="sxs-lookup"><span data-stu-id="7986f-132">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] (`test.sql`) tests the example by exercising the stored procedures.</span></span>  
...
```  
 ...
 <span data-ttu-id="7986f-133">다음 [!INCLUDE[tsql](../../includes/tsql-md.md)] 은 데이터베이스에서 어셈블리, 로그인, 키 및 저장 프로시저를 제거합니다.</span><span class="sxs-lookup"><span data-stu-id="7986f-133">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] removes the assembly, login, key and stored procedures from the database.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="7986f-134">참고 항목</span><span class="sxs-lookup"><span data-stu-id="7986f-134">See Also</span></span>  
 [<span data-ttu-id="7986f-135">공용 언어 런타임 &#40;CLR&#41; 통합에 대한 사용 시나리오 및 예제</span><span class="sxs-lookup"><span data-stu-id="7986f-135">Usage Scenarios and Examples for Common Language Runtime &#40;CLR&#41; Integration</span></span>](../../../2014/database-engine/dev-guide/usage-scenarios-and-examples-for-common-language-runtime-clr-integration.md)  
