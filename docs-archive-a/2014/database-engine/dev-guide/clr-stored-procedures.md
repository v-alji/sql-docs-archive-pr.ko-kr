...
## <a name="requirements-for-clr-stored-procedures"></a>CLR 저장 프로시저에 대한 요구 사항  
 CLR (공용 언어 런타임)에서 저장 프로시저는 어셈블리의 클래스에 대 한 공용 정적 메서드로 구현 됩니다 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] . 정적 메서드는 void로 선언되거나 정수 값을 반환할 수 있습니다. 정적 메서드가 정수 값을 반환하는 경우 반환되는 정수는 프로시저의 반환 코드로 취급됩니다. 예를 들면 다음과 같습니다.  
