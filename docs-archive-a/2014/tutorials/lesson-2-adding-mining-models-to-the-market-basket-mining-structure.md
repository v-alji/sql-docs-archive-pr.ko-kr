...
```  
 ...
 마이닝 모델 열 목록의 첫 번째 열은 마이닝 구조의 키 열이어야 합니다. 그러나 `KEY` 사용을 지정 하기 위해 키 열 뒤에를 입력할 필요는 없습니다. 이는 마이닝 구조를 만들 때 해당 열을 키로 이미 정의했기 때문입니다.  
...
8.  다른 이름 **으로 저장** 대화 상자에서 해당 폴더로 이동한 다음 파일 이름을로 `Default_Association_Model.dmx` 합니다.  
...
8.  다른 이름 **으로 저장** 대화 상자에서 해당 폴더로 이동한 다음 파일 이름을로 `Modified Association_Model.dmx` 합니다.  
