...
7.  **사용 가능한 데이터 원본 뷰**에서 `Call Center` 을 선택 하 고 **다음**을 클릭 합니다.  
...
11. 다음 표와 같이 **키**, **입력**및 확인란을 선택 합니다 `Predict` .  
...
12. **열 내용 및 데이터 형식 지정** 페이지에서 다음 표와 같이 표에 열, 내용 유형 및 데이터 형식이 포함 되어 있는지 확인 한 **후 다음을 클릭 합니다**.  
    ...
    |열|콘텐츠 유형|데이터 형식|  
    |-------------|------------------|----------------|  
    |AutomaticResponses|계속|Long|  
    |AverageTimePerIssue|계속|Long|  
    |호출|계속|Long|  
    |DayOfWeek|불연속|텍스트|  
    |FactCallCenterID|키|Long|  
    |IssuesRaised|계속|Long|  
    |LevelOneOperators|계속|Long|  
    |LevelTwoOperators|계속|Long|  
    |Orders|계속|Long|  
    |ServiceGrade|계속|Double|  
    |Shift|불연속|텍스트|  
    |WageType|불연속|텍스트|  
...
15. **마이닝 모델 이름**에를 입력 한 `Call Center Default NN` 다음 **마침**을 클릭 합니다.  
...
-   **Equal Areas에 의한 범주화**  
 ...
 또는 숫자 값을 사용 하는 것 보다는 서비스 등급을 미리 정의 된 대상 범위로 분류 하는 별도의 파생 열을 추가할 수 있습니다 (예: **Best** (servicegrade \<= 0.05), **Acceptable** (0.10 > servicegrade > **Poor** 0.05) 0.10 >).  
...
3.  **Name** 속성을 찾고 값을로 변경 `ServiceGrade` 합니다.  
...
5.  **마이닝 모델** 탭에서 아무 곳 이나 클릭 합니다.  
     ...
     표를 업데이트 하 여 열 사용 옆에 새 임시 열 별칭을 표시 합니다 `ServiceGrade` . 마이닝 구조와 두 마이닝 모델이 포함된 표는 다음과 같습니다.  
...
2.  HOLDOUT_SEED 매개 변수에 대 한 행에서 **값**아래에 있는 빈 셀을 클릭 하 고을 입력 `1` 합니다. **확인**을 클릭합니다. 구조와 연관된 각 모델에 대해 이 단계를 반복합니다.  
