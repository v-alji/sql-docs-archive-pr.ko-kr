...
# <a name="intermediate-data-mining-tutorial-analysis-services---data-mining"></a>중급 데이터 마이닝 자습서(Analysis Services - 데이터 마이닝)
  [!INCLUDE[msCoName](../includes/msconame-md.md)]는 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 데이터 마이닝 모델을 만들고 작업 하기 위한 통합 환경을 제공 합니다. 데이터 원본에 쉽게 바인딩하고, 여러 모델을 만들어 같은 데이터에 대해 테스트하고, 예측 분석에 사용할 모델을 배포할 수 있습니다.  
