...
ms.locfileid: "87738866"
---
# <a name="calculated-column-representation-tabular"></a><span data-ttu-id="47114-102">계산 열 표현(테이블 형식)</span><span class="sxs-lookup"><span data-stu-id="47114-102">Calculated Column Representation (Tabular)</span></span>
  <span data-ttu-id="47114-103">계산 열은 테이블에 새 열을 만드는 DAX 식이며 구해진 값이 테이블에 저장됩니다.</span><span class="sxs-lookup"><span data-stu-id="47114-103">A calculated column is DAX expression that creates a new column in a table and the obtained values are stored in the table.</span></span> <span data-ttu-id="47114-104">테이블이 처리될 때마다 계산 열 식이 계산됩니다.</span><span class="sxs-lookup"><span data-stu-id="47114-104">The calculated column expression is evaluated every time the table is processed.</span></span>  
  
## <a name="calculated-column-representation"></a><span data-ttu-id="47114-105">계산 열 표현</span><span class="sxs-lookup"><span data-stu-id="47114-105">Calculated Column Representation</span></span>  
  
### <a name="calculated-columns-in-amo"></a><span data-ttu-id="47114-106">AMO의 계산 열</span><span class="sxs-lookup"><span data-stu-id="47114-106">Calculated Columns in AMO</span></span>  
 <span data-ttu-id="47114-107">AMO를 사용하여 테이블 형식 모델 테이블을 관리하는 경우 AMO의 계산 열에 대해 일 대 일 개체 일치 관계가 없습니다.</span><span class="sxs-lookup"><span data-stu-id="47114-107">When using AMO to manage a tabular model table, there is no one-to-one object match for a calculated column in AMO.</span></span> <span data-ttu-id="47114-108">계산 열은 <xref:Microsoft.AnalysisServices.Dimension>의 특성 및 <xref:Microsoft.AnalysisServices.MeasureGroup>의 특성으로 표현됩니다.</span><span class="sxs-lookup"><span data-stu-id="47114-108">A calculated column is represented by an attribute in <xref:Microsoft.AnalysisServices.Dimension> and an attribute in <xref:Microsoft.AnalysisServices.MeasureGroup>.</span></span>  
  
 <span data-ttu-id="47114-109">다음 코드 조각에서는 계산 열을 기존 테이블 형식 모델에 추가하는 방법을 보여 줍니다.</span><span class="sxs-lookup"><span data-stu-id="47114-109">The following code snippet shows how to add a calculated column to an existing tabular model.</span></span> <span data-ttu-id="47114-110">이 코드에서는 AMO 데이터베이스 개체인 newDatabase 및 AMO 큐브 개체인 modelCube를 보유하고 있다고 가정합니다.</span><span class="sxs-lookup"><span data-stu-id="47114-110">The code assumes you have an AMO database object, newDatabase, and an AMO cube object, modelCube.</span></span>  
