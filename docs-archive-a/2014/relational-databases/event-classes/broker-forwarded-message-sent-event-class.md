...
|DBUserName|`nvarchar`|메시지를 보낸 서비스의 Broker 인스턴스 ID입니다.|40|No|  
...
|GUID|`uniqueidentifier`|대화의 대화 ID입니다. 이 식별자는 메시지의 일부로 전송되며 양쪽 대화 상대 간에 공유합니다.|54|No|  
...
|TargetLoginName|`nvarchar`|이 인스턴스가 메시지를 보내는 네트워크 주소입니다. 이 주소는 해당 메시지의 최종 대상과 다를 수도 있습니다.|42|No|  
