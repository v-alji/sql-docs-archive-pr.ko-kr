 ...
 **복구 모델**  
 데이터베이스 복구 모델로 **전체**, **대량 로그** 또는 **단순** 중에서 하나를 지정합니다. 복구 모델에 대한 자세한 내용은 [복구 모델&#40;SQL Server&#41;](../backup-restore/recovery-models-sql-server.md)을 참조하세요.  
 ...
 **재귀적 트리거 설정**  
 ...
 기본적으로 **MSDB**를 제외한 모든 사용자 데이터베이스 및 모든 시스템 데이터베이스에는이 속성이로 설정 되어 있습니다 `False` . **model** 및 **tempdb** 데이터베이스에 대해서는 이 값을 변경할 수 없습니다.  
