...
ms.locfileid: "87729472"
---
# <a name="read-a-filestream-column-to-file-using-ibcpsession-ole-db"></a><span data-ttu-id="1cc59-102">IBCPSession을 사용하여 파일에 대한 FILESTREAM 열 읽기(OLE DB)</span><span class="sxs-lookup"><span data-stu-id="1cc59-102">Read a FILESTREAM Column to File Using IBCPSession (OLE DB)</span></span>
  <span data-ttu-id="1cc59-103">이 예제에서는 IBCPSession 인터페이스를 사용하여 파일에 대한 filestream 열을 읽고 서식 파일을 씁니다.</span><span class="sxs-lookup"><span data-stu-id="1cc59-103">This sample reads a filestream column to a file using the IBCPSession interface and writes a format file.</span></span>  
  
 <span data-ttu-id="1cc59-104">Filestream 기능에 대 한 자세한 내용은 [Filestream Support &#40;OLE DB&#41;](../../native-client/ole-db/filestream-support-ole-db.md)를 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="1cc59-104">For more information on the filestream feature, see [FILESTREAM Support &#40;OLE DB&#41;](../../native-client/ole-db/filestream-support-ole-db.md).</span></span>  
  
## <a name="example"></a><span data-ttu-id="1cc59-105">예제</span><span class="sxs-lookup"><span data-stu-id="1cc59-105">Example</span></span>  
 <span data-ttu-id="1cc59-106">INCLUDE 환경 변수에 sqlncli.h가 들어 있는 디렉터리를 포함해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="1cc59-106">Make sure your INCLUDE environment variable includes the directory that contains sqlncli.h.</span></span>  
  
 <span data-ttu-id="1cc59-107">다음 예제 중 하나를 사용하여 이 예제에서 읽어들이는 테이블을 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="1cc59-107">Use one of the following samples to create the table that this sample reads from:</span></span>  
  
-   [<span data-ttu-id="1cc59-108">ICommandText 매개 변수에 바인딩된 ISequentialStream을 사용하여 FILESTREAM 열에 데이터 전송&#40;OLE DB&#41;</span><span class="sxs-lookup"><span data-stu-id="1cc59-108">Send Data to a FILESTREAM Column Using ISequentialStream Bound to ICommandText Parameter &#40;OLE DB&#41;</span></span>](send-data-to-filestream-isequentialstream-bound-to-icommandtext.md)  
  
-   [<span data-ttu-id="1cc59-109">IRowsetFastUpload를 사용하여 FILESTREAM 열에 데이터 전송&#40;OLE DB&#41;</span><span class="sxs-lookup"><span data-stu-id="1cc59-109">Send Data to a FILESTREAM Column Using IRowsetFastUpload &#40;OLE DB&#41;</span></span>](send-data-to-a-filestream-column-using-irowsetfastupload-ole-db.md)  
  
 <span data-ttu-id="1cc59-110">첫 번째 코드 목록을 복사하고 ISSHelper.h라는 파일로 붙여 넣습니다.</span><span class="sxs-lookup"><span data-stu-id="1cc59-110">Copy the first code listing and paste it into a file called ISSHelper.h.</span></span>  
  
 <span data-ttu-id="1cc59-111">두 번째 코드 목록을 복사하고 ISSHelper.cpp라는 파일로 붙여 넣습니다.</span><span class="sxs-lookup"><span data-stu-id="1cc59-111">Copy the second code listing and paste it into a file called ISSHelper.cpp.</span></span>  
  
 <span data-ttu-id="1cc59-112">세 번째 코드 목록을 복사하고 IBCPSession.cpp라는 파일로 붙여 넣습니다.</span><span class="sxs-lookup"><span data-stu-id="1cc59-112">Copy the third code listing and paste it into a file called IBCPSession.cpp.</span></span>  
  
 <span data-ttu-id="1cc59-113">IBCPSession.cpp, ISSHelper.cpp, ole32.lib 및 oleaut32.lib를 컴파일합니다.</span><span class="sxs-lookup"><span data-stu-id="1cc59-113">Compile IBCPSession.cpp, ISSHelper.cpp, ole32.lib, and oleaut32.lib.</span></span>  
  
 <span data-ttu-id="1cc59-114">이 예제를 실행할 때는 서버 이름이나 server\instance_name 및 작성할 서식 파일 이름을 전달해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="1cc59-114">When this sample is run, you must pass a server name (or server\instance_name) and a name for the format file it will create.</span></span>  
