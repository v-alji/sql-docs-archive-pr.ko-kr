...
ms.locfileid: "87649976"
---
# <a name="send-data-to-a-filestream-column-using-isequentialstream-bound-to-icommandtext-parameter-ole-db"></a><span data-ttu-id="e03c1-102">ICommandText 매개 변수에 바인딩된 ISequentialStream을 사용하여 FILESTREAM 열에 데이터 전송(OLE DB)</span><span class="sxs-lookup"><span data-stu-id="e03c1-102">Send Data to a FILESTREAM Column Using ISequentialStream Bound to ICommandText Parameter (OLE DB)</span></span>
  <span data-ttu-id="e03c1-103">이 예제에서는 ICommandText 매개 변수에 바인딩된 ISequentialStream 인터페이스를 사용하여 filestream 열에 4MB에서 4GB 사이의 데이터를 보냅니다.</span><span class="sxs-lookup"><span data-stu-id="e03c1-103">This sample uses an ISequentialStream interface bound to an ICommandText parameter to send between 4MB and 4GB of data to a filestream column.</span></span>  
  
 <span data-ttu-id="e03c1-104">Filestream 기능에 대 한 자세한 내용은 [Filestream Support &#40;OLE DB&#41;](../../native-client/ole-db/filestream-support-ole-db.md)를 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="e03c1-104">For more information on the filestream feature, see [FILESTREAM Support &#40;OLE DB&#41;](../../native-client/ole-db/filestream-support-ole-db.md).</span></span>  
  
## <a name="example"></a><span data-ttu-id="e03c1-105">예제</span><span class="sxs-lookup"><span data-stu-id="e03c1-105">Example</span></span>  
 <span data-ttu-id="e03c1-106">이 샘플을 컴파일하고 실행하기 전에 FILESTREAM 지원을 설정합니다([FILESTREAM 사용 및 구성](../../blob/enable-and-configure-filestream.md)).</span><span class="sxs-lookup"><span data-stu-id="e03c1-106">Before you compile and run this sample, enable FILESTREAM support ([Enable and Configure FILESTREAM](../../blob/enable-and-configure-filestream.md)).</span></span>  
  
 <span data-ttu-id="e03c1-107">INCLUDE 환경 변수에 sqlncli.h가 들어 있는 디렉터리를 포함해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="e03c1-107">Make sure your INCLUDE environment variable includes the directory that contains sqlncli.h.</span></span>  
  
 <span data-ttu-id="e03c1-108">서버에는 C:\DBFsa라는 디렉터리가 있어야 합니다. 예제에서는 이 디렉터리에 데이터베이스를 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="e03c1-108">The server must have a directory called C:\DBFsa; this is where the sample will create the database.</span></span> <span data-ttu-id="e03c1-109">[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 인스턴스에는 이 위치에 대한 쓰기 권한이 있어야 합니다(예: 로컬 시스템 계정으로 로그온).</span><span class="sxs-lookup"><span data-stu-id="e03c1-109">Your instance of [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] must have write access to this location (for example, log on as a local system account).</span></span>  
  
 <span data-ttu-id="e03c1-110">첫 번째 코드 목록을 복사하고 ISSHelper.h라는 파일로 붙여 넣습니다.</span><span class="sxs-lookup"><span data-stu-id="e03c1-110">Copy the first code listing and paste it into a file called ISSHelper.h.</span></span>  
  
 <span data-ttu-id="e03c1-111">두 번째 코드 목록을 복사하고 ISSHelper.cpp라는 파일로 붙여 넣습니다.</span><span class="sxs-lookup"><span data-stu-id="e03c1-111">Copy the second code listing and paste it into a file called ISSHelper.cpp.</span></span>  
  
 <span data-ttu-id="e03c1-112">세 번째 코드 목록을 복사하고 ICommandUpload.cpp라는 파일로 붙여 넣습니다.</span><span class="sxs-lookup"><span data-stu-id="e03c1-112">Copy the third code listing and paste it into a file called ICommandUpload.cpp.</span></span>  
  
 <span data-ttu-id="e03c1-113">ICommandUpload.cpp, ISSHelper.cpp, ole32.lib 및 oleaut32.lib를 컴파일합니다.</span><span class="sxs-lookup"><span data-stu-id="e03c1-113">Compile ICommandUpload.cpp, ISSHelper.cpp, ole32.lib, and oleaut32.lib.</span></span>  
  
 <span data-ttu-id="e03c1-114">이 예제를 실행할 때는 서버 이름이나 server\instance_name 및 기록할 데이터 양을 나타내는 4MB(0x400001)에서 4GB(0xFFFFFFFF) 사이의 값을 전달해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="e03c1-114">When you run this sample, you must pass the name of a server, or a server\instance_name, as well as a value between 4 MB (0x400001) and 4 GB (0xFFFFFFFF) indicating the amount of data to write.</span></span>  
  
 <span data-ttu-id="e03c1-115">네 번째([!INCLUDE[tsql](../../../includes/tsql-md.md)]) 코드 목록은 이 예제에서 만든 데이터베이스를 삭제합니다.</span><span class="sxs-lookup"><span data-stu-id="e03c1-115">The fourth ([!INCLUDE[tsql](../../../includes/tsql-md.md)]) code listing deletes the database created by this sample.</span></span>  
