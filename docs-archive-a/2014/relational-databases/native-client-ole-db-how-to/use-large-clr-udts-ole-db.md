...
ms.locfileid: "87729439"
---
# <a name="use-large-clr-udts-ole-db"></a><span data-ttu-id="9a024-102">큰 CLR UDT 사용(OLE DB)</span><span class="sxs-lookup"><span data-stu-id="9a024-102">Use Large CLR UDTs (OLE DB)</span></span>
  <span data-ttu-id="9a024-103">이 예제에서는 결과 집합에서 큰 사용자 정의 형식의 행을 인출하는 방법을 보여 줍니다.</span><span class="sxs-lookup"><span data-stu-id="9a024-103">This sample shows how to fetch rows with large user-defined types from a result set.</span></span> <span data-ttu-id="9a024-104">자세한 내용은 [큰 CLR 사용자 정의 형식&#40;OLE DB&#41;](../native-client/ole-db/large-clr-user-defined-types-ole-db.md)을 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="9a024-104">For more information, see [Large CLR User-Defined Types &#40;OLE DB&#41;](../native-client/ole-db/large-clr-user-defined-types-ole-db.md).</span></span> <span data-ttu-id="9a024-105">이 예제는 [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] 이상에서만 작동합니다.</span><span class="sxs-lookup"><span data-stu-id="9a024-105">This sample works with [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] or later.</span></span>  
  
## <a name="example"></a><span data-ttu-id="9a024-106">예제</span><span class="sxs-lookup"><span data-stu-id="9a024-106">Example</span></span>  
 <span data-ttu-id="9a024-107">이 예제에는 두 개의 프로젝트가 포함되어 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9a024-107">This sample contains two projects.</span></span> <span data-ttu-id="9a024-108">첫 번째 프로젝트는 C# 원본 코드에서 어셈블리(DLL)를 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="9a024-108">One project creates an assembly (DLL) from C# source code.</span></span> <span data-ttu-id="9a024-109">이 어셈블리에는 CLR 유형이 포함되어 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9a024-109">This assembly contains the CLR type.</span></span> <span data-ttu-id="9a024-110">테이블이 데이터베이스에 추가됩니다.</span><span class="sxs-lookup"><span data-stu-id="9a024-110">A table will be added to the database.</span></span> <span data-ttu-id="9a024-111">테이블의 열은 어셈블리에 정의된 유형으로 지정됩니다. 기본적으로 이 예제에서는 master 데이터베이스를 사용합니다.</span><span class="sxs-lookup"><span data-stu-id="9a024-111">A column in the table will be of a type defined in the assembly, By default, this sample will use the master database.</span></span> <span data-ttu-id="9a024-112">두 번째 프로젝트는 테이블에서 데이터를 읽는 네이티브 C 애플리케이션입니다.</span><span class="sxs-lookup"><span data-stu-id="9a024-112">The second project is a native C application that reads data from the table.</span></span>  
  
 <span data-ttu-id="9a024-113">첫 번째(C#) 코드 목록을 DLL로 컴파일합니다.</span><span class="sxs-lookup"><span data-stu-id="9a024-113">Compile the first (C#) code listing to a DLL.</span></span>  <span data-ttu-id="9a024-114">그리고 나서 DLL을 C 드라이브의 루트 디렉터리에 복사합니다.</span><span class="sxs-lookup"><span data-stu-id="9a024-114">Then, copy the DLL to the root directory of the C drive.</span></span>  
  
 <span data-ttu-id="9a024-115">두 번째([!INCLUDE[tsql](../../includes/tsql-md.md)]) 코드 목록을 실행하여 어셈블리를 master 데이터베이스에 추가합니다.</span><span class="sxs-lookup"><span data-stu-id="9a024-115">Execute the second ([!INCLUDE[tsql](../../includes/tsql-md.md)]) code listing to add the assembly to the master database.</span></span>  
  
 <span data-ttu-id="9a024-116">ole32.lib oleaut32.lib를 사용하여 컴파일하고 세 번째(C++) 코드 목록을 실행합니다.</span><span class="sxs-lookup"><span data-stu-id="9a024-116">Compile with ole32.lib oleaut32.lib and execute the third (C++) code listing.</span></span> <span data-ttu-id="9a024-117">이 애플리케이션은 컴퓨터의 기본 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 인스턴스에 연결됩니다.</span><span class="sxs-lookup"><span data-stu-id="9a024-117">This application connects to your computer's default [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance.</span></span> <span data-ttu-id="9a024-118">일부 Windows 운영 체제에서는 (localhost) 또는 (local)을 해당 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 인스턴스의 이름으로 변경해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a024-118">On some Windows operating systems, you will need to change (localhost) or (local) to the name of your [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance.</span></span> <span data-ttu-id="9a024-119">명명된 인스턴스에 연결하려면 연결 문자열을 L"(local)"에서 L"(local)\\\name"으로 변경합니다. 여기서 name은 명명된 인스턴스입니다.</span><span class="sxs-lookup"><span data-stu-id="9a024-119">To connect to a named instance, change the connection string from L"(local)" to L"(local)\\\name" , where name is the named instance.</span></span> <span data-ttu-id="9a024-120">기본적으로 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express는 명명된 인스턴스에 설치됩니다.</span><span class="sxs-lookup"><span data-stu-id="9a024-120">By default, [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express installs to a named instance.</span></span> <span data-ttu-id="9a024-121">INCLUDE 환경 변수에 sqlncli.h가 들어 있는 디렉터리를 포함해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a024-121">Make sure your INCLUDE environment variable includes the directory that contains sqlncli.h.</span></span>  
  
 <span data-ttu-id="9a024-122">네 번째([!INCLUDE[tsql](../../includes/tsql-md.md)]) 코드 목록을 실행하여 master 데이터베이스의 어셈블리를 삭제합니다.</span><span class="sxs-lookup"><span data-stu-id="9a024-122">Execute the fourth ([!INCLUDE[tsql](../../includes/tsql-md.md)]) code listing to delete the assembly to the master database.</span></span>  
