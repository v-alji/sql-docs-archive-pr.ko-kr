...
## <a name="arguments"></a>인수  
 RAW[('*ElementName*')]  
 쿼리 결과를 사용하여 결과 집합의 각 행을 요소 태그로 일반 식별자 \<row />를 갖는 XML 요소로 변환합니다. 이 지시어를 사용하는 경우 필요에 따라 행 요소에 대한 이름을 지정할 수 있습니다. 결과 XML은 각 행에 대해 행 요소가 생성될 때 지정된 *ElementName* 을 사용합니다. 자세한 내용은 [FOR XML에서 RAW 모드 사용](use-raw-mode-with-for-xml.md)을 참조하세요.  
