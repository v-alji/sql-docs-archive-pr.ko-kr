...
ms.locfileid: "87659969"
---
# <a name="ibcpsessionbcpdone-ole-db"></a><span data-ttu-id="8a49a-102">IBCPSession::BCPDone(OLE DB)</span><span class="sxs-lookup"><span data-stu-id="8a49a-102">IBCPSession::BCPDone (OLE DB)</span></span>
  <span data-ttu-id="8a49a-103">[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]에 보낼 나머지 행을 커밋합니다.</span><span class="sxs-lookup"><span data-stu-id="8a49a-103">Commits the remaining rows to be sent to [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)].</span></span>  
  
## <a name="syntax"></a><span data-ttu-id="8a49a-104">구문</span><span class="sxs-lookup"><span data-stu-id="8a49a-104">Syntax</span></span>  
...
## <a name="remarks"></a><span data-ttu-id="8a49a-105">설명</span><span class="sxs-lookup"><span data-stu-id="8a49a-105">Remarks</span></span>  
 <span data-ttu-id="8a49a-106">**BCPDone** 메서드를 호출한 후에는 [IBCPSession](ibcpsession-ole-db.md) 인터페이스에 대해 다른 작업을 호출할 수 없습니다.</span><span class="sxs-lookup"><span data-stu-id="8a49a-106">No other operation can be called on the [IBCPSession](ibcpsession-ole-db.md) interface after calling the **BCPDone** method.</span></span> <span data-ttu-id="8a49a-107">[IBCPSession::BCPInit](ibcpsession-bcpinit-ole-db.md) 메서드를 호출하여 새 대량 복사 작업을 시작하는 것만 가능합니다.</span><span class="sxs-lookup"><span data-stu-id="8a49a-107">The only possibility is to call the [IBCPSession::BCPInit](ibcpsession-bcpinit-ole-db.md) method to initiate a new bulk copy operation.</span></span> <span data-ttu-id="8a49a-108">이 동작은 [IRowsetFastLoad::Commit](irowsetfastload-commit-ole-db.md) 메서드를 호출하는 것과 유사합니다.</span><span class="sxs-lookup"><span data-stu-id="8a49a-108">This is similar to calling the [IRowsetFastLoad::Commit](irowsetfastload-commit-ole-db.md) method.</span></span>  
  
## <a name="return-code-values"></a><span data-ttu-id="8a49a-109">반환 코드 값</span><span class="sxs-lookup"><span data-stu-id="8a49a-109">Return Code Values</span></span>  
 <span data-ttu-id="8a49a-110">S_OK</span><span class="sxs-lookup"><span data-stu-id="8a49a-110">S_OK</span></span>  
 <span data-ttu-id="8a49a-111">메서드가 성공했습니다.</span><span class="sxs-lookup"><span data-stu-id="8a49a-111">The method succeeded.</span></span>  
  
 <span data-ttu-id="8a49a-112">E_UNEXPECTED</span><span class="sxs-lookup"><span data-stu-id="8a49a-112">E_UNEXPECTED</span></span>  
 <span data-ttu-id="8a49a-113">예기치 않은 메서드가 호출되었습니다.</span><span class="sxs-lookup"><span data-stu-id="8a49a-113">The call to the method was unexpected.</span></span> <span data-ttu-id="8a49a-114">예를 들어 이 메서드를 호출하기 전에 **BCPInit** 메서드를 호출하지 않았습니다.</span><span class="sxs-lookup"><span data-stu-id="8a49a-114">For example, the **BCPInit** method was not called before calling this method.</span></span>  
  
## <a name="example"></a><span data-ttu-id="8a49a-115">예제</span><span class="sxs-lookup"><span data-stu-id="8a49a-115">Example</span></span>  
 <span data-ttu-id="8a49a-116">이 예제에서는 **IBCPSession** 인터페이스를 사용하는 방법을 보여 줍니다.</span><span class="sxs-lookup"><span data-stu-id="8a49a-116">This sample shows how to use the **IBCPSession** interface.</span></span>  
  
 <span data-ttu-id="8a49a-117">이 예제를 실행하기 전에 다음 [!INCLUDE[tsql](../../includes/tsql-md.md)] 을 실행해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="8a49a-117">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] must be executed before running this sample:</span></span>  
...
```  
 ...
 <span data-ttu-id="8a49a-118">이 예제는 master 데이터베이스에 연결됩니다.</span><span class="sxs-lookup"><span data-stu-id="8a49a-118">The sample connects to the master database.</span></span>  
  
 <span data-ttu-id="8a49a-119">예제를 실행하면 프로젝트 디렉터리에 outfile.dat 파일이 생성됩니다.</span><span class="sxs-lookup"><span data-stu-id="8a49a-119">When the sample runs, it creates outfile.dat in the project directory.</span></span> <span data-ttu-id="8a49a-120">outfile.dat 파일에는 기본(이진) 모드로 테이블에서 복사한 데이터가 포함됩니다.</span><span class="sxs-lookup"><span data-stu-id="8a49a-120">The outfile.dat contains the data copied from the table in native (binary) mode.</span></span>  
  
 <span data-ttu-id="8a49a-121">BCP를 사용하여 이 데이터를 테이블에 다시 추가할 수 있습니다. 예를 들어 다음 명령을 사용할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="8a49a-121">You could use BCP to add this data back into the table with the following command:</span></span>  
  
 <span data-ttu-id="8a49a-122">**bcp master..fltest in outfile.dat -n -T -S** *server*</span><span class="sxs-lookup"><span data-stu-id="8a49a-122">**bcp master..fltest in outfile.dat -n -T -S** *server*</span></span>  
  
 <span data-ttu-id="8a49a-123">이 예제를 컴파일할 때 sqlncli11.lib를 지정해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="8a49a-123">You will need to specify sqlncli11.lib when compiling this sample.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="8a49a-124">참고 항목</span><span class="sxs-lookup"><span data-stu-id="8a49a-124">See Also</span></span>  
 <span data-ttu-id="8a49a-125">[IBCPSession &#40;OLE DB&#41;](ibcpsession-ole-db.md) </span><span class="sxs-lookup"><span data-stu-id="8a49a-125">[IBCPSession &#40;OLE DB&#41;](ibcpsession-ole-db.md) </span></span>  
 [<span data-ttu-id="8a49a-126">대량 복사 작업 수행</span><span class="sxs-lookup"><span data-stu-id="8a49a-126">Performing Bulk Copy Operations</span></span>](../native-client/features/performing-bulk-copy-operations.md)  
