...
7.  **스키마의 메타데이터** 데이터 공급자는 쿼리와 별도로 스키마 쿼리 명령을 실행하여 데이터 세트 필드 컬렉션에 대한 메타데이터를 검색합니다. 예를 들어 [!INCLUDE[tsql](../../../includes/tsql-md.md)] `SELECT` 문은 데이터베이스 테이블에 대 한 열 이름을 반환 합니다. 데이터 세트 필드 컬렉션을 보려면 보고서 데이터 창을 사용하여 데이터 세트를 확장합니다.  
