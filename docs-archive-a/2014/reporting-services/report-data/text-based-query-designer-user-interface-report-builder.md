...
#### <a name="example"></a>예제
 다음 쿼리는 [!INCLUDE[ssSampleDBobject](../../../includes/sssampledbobject-md.md)] **2008** 데이터베이스 `ContactType` 테이블에서 스키마에 대 한 마지막 이름 목록을 반환 합니다 `Person` .
...
#### <a name="example"></a>예제
 다음 쿼리는 [!INCLUDE[ssSampleDBobject](../../../includes/sssampledbobject-md.md)] **2008** 저장 프로시저를 호출 합니다 `uspGetWhereUsedProductID` . 쿼리를 실행할 때 제품 ID 번호 매개 변수에 대한 값을 입력해야 합니다.
...
#### <a name="example"></a>예제
 OLE DB 데이터 원본 유형의 경우 다음 데이터 집합 쿼리는 2008 데이터베이스에 있는 모든 연락처 유형에 대 한 결과 집합을 반환 합니다 [!INCLUDE[ssSampleDBobject](../../../includes/sssampledbobject-md.md)] **2008** .
