...
-   **텍스트 실행을 사용 하는 식:** 텍스트, **평균 판매량**, 식, 및 `=Avg(Fields!YTDPurchase.Value, "Sales)` 텍스트, **페이지 번호**및 식`=Globals!PageNumber`  
...
-   테이블 릭 스 (테이블, 행렬 또는 목록)의 정적 머리글 행에 있는 RepeatOnNewPage 속성을로 설정 하더라도 Word에서는 두 페이지 이상에서 머리글 행을 반복 하지 않습니다 `True` . 보고서에 명시적 페이지 나누기를 정의하여 머리글 행이 새 페이지에도 표시되게 할 수 있습니다. 그러나 Word로 내보내 렌더링한 보고서에는 Word 자체의 고유한 페이지 매김 방식이 적용되므로 그 결과가 예상과 다를 수 있으며 머리글 행이 반복되지 않을 수 있습니다. 정적 머리글 행은 열 머리글을 포함하는 행입니다.  
