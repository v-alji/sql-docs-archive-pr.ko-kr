...
# <a name="removing-a-rendering-extension"></a>렌더링 확장 프로그램 제거
  [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)]렌더링 확장 프로그램을 제거 하려면 `Extension` **%ProgramFiles%\Microsoft SQL Server \ MSRS10_50에 있는 rsreportserver.config 파일에서 렌더링 확장 프로그램에 대 한 요소를 제거 하면 됩니다. \<Instance Name> \Reporting Services\ReportServer** 폴더 보고서 디자이너 및 보고서 서버에 대 한 항목을 만든 경우 `Extension` [Rsreportdesigner.config 구성 파일](../../report-server/rsreportdesigner-configuration-file.md) 에서도 요소를 제거 합니다. 구성 정보를 제거한 후에는 구성 요소에서 렌더링 확장 프로그램을 사용할 수 없습니다.  
