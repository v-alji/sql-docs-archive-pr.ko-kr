...
|`LinkTarget`|보고서에서 하이퍼링크에 대한 대상입니다. Window_name와 같이 창의 이름을 제공 하 여 창이 나 프레임을 대상으로 `LinkTarget` = *window_name*하거나 = _blank를 사용 하 여 새 창을 대상으로 지정할 수 있습니다 `LinkTarget` . 기타 유효한 대상 이름에는 _self, _parent 및 _top이 포함됩니다.|  
...
|**StreamRoot(\*)**|보고서 서버에서 반환된 HTML 보고서에서 IMG 요소의 **src** 특성 값 앞에 접두사를 추가하는 데 사용되는 경로입니다. 기본적으로 보고서 서버에서 경로가 제공됩니다. 이 설정을 사용 하 여 보고서의 이미지에 대 한 루트 경로 (예: **http:// \<servername> /resources/companyimages**)를 지정할 수 있습니다.|  
