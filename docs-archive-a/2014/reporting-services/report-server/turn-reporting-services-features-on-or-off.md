...
2.  개체 탐색기에서 노드를 마우스 오른쪽 단추로 클릭 하 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 고 **정책**을 가리킨 다음 **패싯**을 클릭 합니다.  
  
3.  **패싯** 목록에서 **Reporting Services에 대한 노출 영역**을 선택합니다.  
  
4.  **패싯 속성**아래에서 다음을 수행합니다.  
  
    -   보고서 서버 웹 서비스를 활성화 하려면 **Webserviceandhttpaccessenabled** 를로 설정 `True` 합니다.  
  
    -   보고서 서버 웹 서비스를 해제 하려면 **Webserviceandhttpaccessenabled** 를로 설정 `False` 합니다.  
...
4.  **패싯 속성**아래에서 다음을 수행합니다.  
    ...
    -   예약 된 이벤트 및 배달을 설정 하려면 **ScheduleEventsAndReportDeliveryEnabled** 을로 설정 `True` 합니다.  
  
    -   예약 된 이벤트 및 배달을 해제 하려면 **ScheduleEventsAndReportDeliveryEnabled** 을로 설정 `False` 합니다.  
...
4.  **패싯 속성**아래에서 다음을 수행합니다.  
    ...
    -   보고서 관리자 설정 하려면 **Reportmanagerenabled** 를로 설정 `True` 합니다.  
  
    -   보고서 관리자 해제 하려면 **Reportmanagerenabled** 를로 설정 `False` 합니다.  
